...
# 1. Active Mode Check
if [[ "${1:-}" != "--force" ]]; then
  if ! /home/dave/bin/trmnl-set-display-mode status | grep -q "TRMNL Mode: jen_commute"; then
...
python3 "${SCRIPT_DIR}/render_jen_commute.py"

# 4. Copy image to LaraPaper container
SIDECAR_IMAGE_PATH="${SCRIPT_DIR}/tmp/sidecar_jen_commute_next.png"
SIDECAR_IMAGE_NAME="sidecar_jen_commute_next"
...
$metadata = json_encode([
    ...
    'palette_id' => 10,
    'mime_type' => 'image/png',
    'renderer' => 'trmnl-display jen commute colour sidecar',
]);
...
echo "Jen Commute sidecar refresh complete"
